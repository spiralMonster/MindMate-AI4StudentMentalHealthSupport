...
class RelaxationChain:
    ...
    {jokes}
    """
        self.joke_prompt = PromptTemplate.from_template(input_variable=['situation', 'jokes'], template=temp3)
    ...
    """
        ...
        self.motivation_prompt = PromptTemplate.from_template(input_variable=['situation', 'motivation'],
                                                              template=temp4)
    ...
    def internet_joke(self):
        ...
        def internet_motivation(self):
            motiv = self.tool.invoke({
                                         'query': 'Give me some real life examples which were not good in academics but acheive a great success'})
    ...
    def run(self):
        # The question to be returned when the relaxation chain is over.
        ...
        while True:
            ...
            if activity['response'].strip() == 'Joke':
                ...
                motivation = self.motivator_chain.invoke(
                    {'situation': self.situation, 'motivation': motivation_context})
                ...
                print("AI: Anything else you want to ask about?: ")
                return_question = input("User: ")
                break
